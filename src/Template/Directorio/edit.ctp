...
$(document).ready(function(){
  ...
  function genera_municipio(llave){
    $.getJSON(getUrl() + '/Directorio/obtienemunicipios?id='+llave, function(data, status){
        ...
        $("#localidad").empty()
        $('#municipio').val("<?php echo $directorio->municipio?>").trigger('change');
  ...
  function genera_localidad(llave){
    $.getJSON(getUrl() + '/Directorio/obtienelocalidades?id='+llave, function(data, status){
        ...
        $('#localidad').val("<?php echo $directorio->id_localidad?>").trigger('change.select2');
    });
  }
...
<div class="cell auto-size padding20 bg-white" id="cell-content">
    <h1 class="text-light">Editar<span class="mif-profile place-right"></span></h1>
    ...
    <fieldset>
        <legend><?php echo __('Editar datos de: '); ?><?php echo utf8_decode($directorio->nombre_completo)?></legend>
        <!-- Nombre -->
              ...
              <div class="form-group">
                <div class="col-sm-10">
                  ...
                  echo $this->Form->select(
                    'estado',$estados,
                    array('id'=>'estado','name'=>'estado','default'=>$directorio->estado,'data-role'=>'select','style'=>'width:100%;','empty'=>false));
                    ?>
                  </div>
                </div>
                <!-- Estado-->
                ...
                <div class="col-md-4">
                  ...
                  <div class="form-group">
                    <div class="col-sm-10">
                      ...
                      echo $this->Form->select(
                        'localidad',[],
                        array('id'=>'localidad','name'=>'localidad','data-role'=>'select','style'=>'width:100%;','empty'=>false,'required'));
                        ?>
                      </div>
                    </div>
                    <!-- Localidad-->
